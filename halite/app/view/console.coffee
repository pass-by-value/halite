...
mainApp.controller 'ConsoleCtlr', ['$scope', '$location', '$route', '$q', '$filter',
    ...
    ($scope, $location, $route, $q, $filter, $templateCache, $http, Configuration,
    ...
    Commander, Pagerage, SaltApiSrvc, SaltApiEvtSrvc, SessionStore ) ->
        ...
        $scope.startJob = (result, cmd) ->
            # console.log "in stasrtjob with #{result} and #{cmd}"
        ...
        class Command
          constructor: (scope, command, target=null) ->
            @scope = scope
          ...
          _getResult: (httpResponse) ->
            httpResponse.return[0]

          _getJID: (httpResponse) ->
            @_getResult.jid
          ...
          executeAsync: () ->
            ...
            SaltApiSrvc.run(@scope, @command)
            .success (data, status, headers, config) ->
              job = self.scope.startJob(self._getResult(data), self.command)
              job.commit($q).then (donejob) ->
                self.processSuccess(donejob)
                return true
              , (error) ->
        ...
        class ApacheStatusCommand extends Command
          constructor: (scope, target) ->
            super(scope, 'apache.server_status', target)
            @command.mode = 'async'

          assignResult: (apacheCmdResult) ->
            @scope.minions.get(@command.tgt)?.apache = apacheCmdResult
            return true

          processSuccess: (data) ->
            @assignResult(@processApacheResult(@scope.jobs.get(data.jid).results)[0])
            return true
          ...
          processApacheResult: (minion) ->
            retval = []
            for resulter in minion.values()
              fail = resulter.fail or resulter.return == 'error'
              success = resulter.success
              ...
              data =
                ...
                id: resulter.id
                retcode: resulter.retcode
                uptime: resulter.return?.Uptime
              retval.push data
            return retval

        $scope.submitApacheStatusCheck = (mid) ->
          apacheCommand = new ApacheStatusCommand($scope, mid)
          apacheCommand.executeAsync()
          return true
